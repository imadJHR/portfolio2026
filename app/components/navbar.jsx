...
export default function Navbar({ lang }) {
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-50 px-3 pt-3" dir={isRTL ? "rtl" : "ltr"}>
      <motion.div initial={{ y: -18, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: .5 }} className={`container relative overflow-hidden rounded-full border transition-all duration-500 ${isScrolled ? "border-[var(--border)] bg-[var(--bg-card)] shadow-[var(--shadow-md)] backdrop-blur-2xl" : "border-transparent bg-transparent"}`}>
        ...
        <nav className="flex h-16 items-center justify-between gap-3">
          <Link href={lang === "fr" ? "/fr" : "/ar"} className="flex items-center gap-2.5">
            {mounted && <Image src="/dark.png" alt="Nemsi" width={38} height={38} className="h-9 w-auto rounded-xl" priority />}
            <span className="text-base font-black tracking-tight sm:text-lg">Nemsi<span className="gradient-text ml-1">Media</span></span>
          </Link>

          <div className="hidden items-center gap-1 rounded-full border border-[var(--border)] bg-[var(--bg-surface)] p-1 backdrop-blur-xl lg:flex">
          ...
          <div className="hidden items-center gap-2 lg:flex">
            ...
            <SpecularButton onClick={toggleTheme} variant="ghost" size="icon" aria-label="Toggle theme">{theme === "dark" ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</SpecularButton>
          ...
          <div className="flex items-center gap-1.5 lg:hidden">
            ...
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              ...
              <SheetContent side={isRTL ? "right" : "left"} className="w-[310px] border-[var(--border)] bg-[var(--bg)] p-6 text-[var(--text)]">
                ...
                <div className="mt-8 flex flex-col gap-3">
                  ...
                  <Link href={`/${otherLang}`} className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] px-4 py-3 font-semibold text-[var(--text-secondary)]">{otherLang === "ar" ? "العربية" : "Français"}</Link>
