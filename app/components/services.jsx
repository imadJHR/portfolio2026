...
export function Services({ lang }) {
  ...
  return <section id="services" className="nm-services section" dir={isRTL ? "rtl" : "ltr"}><div className="container">
    ...
    <div className="nm-service-grid">{services.map(([slug, title, description], index) => <Link href={`/${lang}/services/${slug}`} className="nm-service-card" key={slug}><span className="nm-service-card__media"><Image src={serviceImages[index]} alt={isRTL ? `صورة توضيحية لخدمة ${title}` : `Illustration du service ${title}`} width={1122} height={1402} sizes="(max-width: 700px) 100vw, (max-width: 1100px) 50vw, 25vw" /></span><span className="nm-service-card__number">{String(index + 1).padStart(2, "0")}</span><div className="nm-service-card__content"><h3>{title}</h3><p>{description}</p></div><span className="nm-service-card__link">{isRTL ? "اكتشف" : "Découvrir"}<ArrowUpRight aria-hidden="true" /></span></Link>)}</div>
  </div></section>
}
