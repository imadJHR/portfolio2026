...
export function Services({ lang }) {
  ...
  return <section id="services" className="nm-services section" dir={isRTL ? "rtl" : "ltr"}><div className="container">
    ...
    <div className="nm-service-grid">{services.map(([slug, title, description], index) => <Link href={`/${lang}/services/${slug}`} className="nm-service-card" key={slug}><span className="nm-service-card__media" aria-hidden="true"><Image src={serviceImages[index]} alt="" fill sizes="(max-width: 700px) 100vw, (max-width: 1100px) 50vw, 25vw" /></span><span className="nm-service-card__number">{String(index + 1).padStart(2, "0")}</span><div className="nm-service-card__content"><h3>{title}</h3><p>{description}</p></div><span className="nm-service-card__link">{isRTL ? "اكتشف" : "Découvrir"}<ArrowUpRight aria-hidden="true" /></span></Link>)}</div>
  </div></section>
}
