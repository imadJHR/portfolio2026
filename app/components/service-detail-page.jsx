...
export function ServiceDetailPage({ slug, content, relatedServices, lang }) {
  ...
  const lead = () => openWhatsApp(isRTL ? `مرحبا، أود مناقشة خدمة ${content.name}.` : `Bonjour, je souhaite discuter du service ${content.name}.`, "service_detail_page", { service: slug, language: lang })

  return <div dir={isRTL ? "rtl" : "ltr"}><Navbar lang={lang} /><main>
    <section id="home" className="nm-service-hero"><div className="container"><nav><Link href={`/${lang}/services`}>{copy.back}</Link><span>/</span><span>{content.name}</span></nav><div className="nm-service-hero__grid"><div><p className="nm-kicker">{content.name}</p><h1>{content.title}</h1></div><div><strong>{content.highlight}</strong><p>{content.description}</p><button className="nm-button nm-button--primary" onClick={lead}>{copy.project}<ArrowUpRight /></button></div></div></div></section>
    ...
    <section className="nm-service-cta"><div className="container"><div><h2>{copy.final}</h2><p>{copy.finalText}</p></div><button className="nm-button nm-button--primary" onClick={lead}>{copy.project}<ArrowUpRight /></button></div></section>
