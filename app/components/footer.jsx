      ...
      <div className="container relative z-10">
        <div className="glass-card overflow-hidden p-6 sm:p-8 lg:p-10">
          ...
          <div className="grid grid-cols-1 gap-8 border-y border-[var(--border)] py-8 md:grid-cols-2 lg:grid-cols-12">
            ...
            <div className="lg:col-span-3"><p className="mb-4 text-xs font-black uppercase tracking-[.2em] text-[var(--text-muted)]">Contact</p><div className="space-y-3"><a href="tel:+212645288216" className="flex items-center gap-2 text-sm text-[var(--text-secondary)] hover:text-[var(--text)]"><Phone className="h-4 w-4 text-[var(--brand-hover)]" />+212 6 45 28 82 16</a><a href="mailto:contact@nemsimedia.ma" className="flex items-center gap-2 text-sm text-[var(--text-secondary)] hover:text-[var(--text)]"><Mail className="h-4 w-4 text-[var(--brand-hover)]" />contact@nemsimedia.ma</a><div className="flex items-center gap-2 text-sm text-[var(--text-secondary)]"><MapPin className="h-4 w-4 text-[var(--brand-hover)]" />{isRTL ? "الدار البيضاء، المغرب" : "Casablanca, Maroc"}</div></div><SpecularLink href="mailto:contact@nemsimedia.ma?subject=Demande%20de%20projet" variant="ghost" size="sm" className="mt-5 w-full"><Mail className="h-4 w-4" />{isRTL ? "راسلنا بالبريد" : "Écrire par email"}</SpecularLink></div>
          </div>
          <div className="flex flex-col items-center justify-between gap-4 pt-6 sm:flex-row"><p className="text-xs text-[var(--text-muted)]">© {year} Nemsi Media. {isRTL ? "جميع الحقوق محفوظة." : "Tous droits réservés."}</p><div className="flex items-center gap-4 text-xs text-[var(--text-muted)]"><span className="flex items-center gap-1.5">{isRTL ? "صُنع بـ" : "Fait avec"} <Heart className="h-3 w-3 text-red-400" /> {isRTL ? "في المغرب" : "au Maroc"}</span><SpecularButton onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} variant="ghost" size="icon" aria-label={isRTL ? "العودة إلى الأعلى" : "Retour en haut"}><ArrowUp className="h-4 w-4" /></SpecularButton></div></div>
        </div>
      </div>
    </footer>
  )
